...
async function createToken (request, reply) {
    ...
    // Retrieve secret key id
    let secret_id = await getSecret(secret);
    // Both ids must be present in each table, if there is no one then an error message should be thrown.
    if(!secret_id || !user_id){
        return response.badRequest('','Your Token or Secret key is wrong!', reply);
    }

    // Create a random string of approximately 20 characters
    let id = crypto.randomBytes(25).toString('hex');

    // Make today's date + 30 days ahead for the active period of the token..
